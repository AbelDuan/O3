#!/usr/bin/env python3
"""修复越狱 APK 的 SELinux 副作用（v6 —— 目标地址前移 7 字节）。
...
OUT_UNSIGNED = os.path.join(WORK, "unsigned-sel6.apk")
OUT_SIGNED = os.path.join(WORK, "ennea-o3v1-18sel-sel6.apk")
...
DEX_OLD, DEX_NEW = b"\x30", b"\x29"   # 0x...9630 -> 0x...9629（前移 7）
SO_OFF = 0xE0EC - 0x4000          # .text 映射 vaddr = fileoff + 0x4000
SO_OLD, SO_NEW = 0x928D39E9, 0x928D3AC9   # movn x9,#0x69cf -> #0x69d6（前移 7）
...
def main():
    ...
    print("  so   movn x9,#0x69cf -> #0x69d6  (file %#x)" % SO_OFF)

    so_path = os.path.join(WORK, "libennea64560.sel6.so")
