#!/usr/bin/env python3
"""修复越狱 APK 的 SELinux 副作用（v4 —— 目标地址前移 3 字节）。
...
OUT_UNSIGNED = os.path.join(WORK, "unsigned-sel4.apk")
OUT_SIGNED = os.path.join(WORK, "ennea-o3v1-18sel-sel4.apk")
...
DEX_OLD, DEX_NEW = b"\x30", b"\x2d"   # 0x...9630 -> 0x...962d
SO_OFF = 0xE0EC - 0x4000          # .text 映射 vaddr = fileoff + 0x4000
SO_OLD, SO_NEW = 0x928D39E9, 0x928D3A49   # movn x9,#0x69cf -> #0x69d2
...
def main():
    ...
    print("  so   movn x9,#0x69cf -> #0x69d2  (file %#x)" % SO_OFF)

    so_path = os.path.join(WORK, "libennea64560.sel4.so")
